...
* Validation report
A validation or log report is created and saved in the ~Log~ sub-folder
...
* Data report
  ...
  1) If the insertion is in the original questionnaire format
     * if ADM names exists in database they must equal to the one in
      ...
      accessing the database directly, and if you try to reinsert the original questionnaire with reordered of changed region names, it will fail.
  2) General
