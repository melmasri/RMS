...
extract them using one of the following methods
    ...
    2. *Wildcard*: since the huge list of indicators, users can
       ...
       * To percentage of female teachers in ISCED levels 1, 2, and 3,
...
conditions are saved and also printed to the [[Status frame and Log subfolder][Status]] frame:
    1) If the import is in the original questionnaire format
       ...
       * If ADM names exists in database they must equal to the one in
         ...
         accessing the database directly. *(/Note I didn't see this in
         the code in adm1_names_test(), can you check/)*.
       * All checks in the ~Checking sheet~ are copied to the
          validation report, and ignored in this step. */(I didn't see
         this step)/*
       * Reference year is filled and an integer, from cell ~M14~ in
         Policy information sheet.
       * Correct number of sheets. *(/didn't see this step/)*
    2) If the import is *not* in the original questionnaire format
       ...
       * Configuration panel is properly filled.
         ...
         * Country code is proper.
         * Numeric year value.
         * Mode defined as "Edit".
         * Series defined as REP, OBS or EST,
         * */Do you check country names and co_code/ ?*
    ...
    3) General checks: /*Not sure about this either*/
       ...
       * values_test: Not very clear function I tried to read it.??
...
* TODO 
  ...
  + [ ] check all validation steps are correct.
  + [ ] complete Data report section, mainly what happens for less
    than check and region total when data has m ,n ,a,z, or x.
  + [ ] Compelete Calculating indicators section.


Check from file
    + nadm1_test: no. of is filled with a positive integer.
    + adm1_label_test: Check if the type of ADM is provided ( new ques only)
    + adm1_names_test: check if ADM names are filled (new ques only)
    + reference_year_test: check that referece year is filled (new
      ques only) cell M14 in Policy infomation.
    + country_name_test: Check that country name exit
    + number_of_sheets_test: correct no of sheets are submitions (new
      ques only)
    + edited_configuration_part_test: configuration panel exists and
      edit mode and all values are properly filled. 
    + values_test: Not very clear
