...
This document explains how to use the Regional Module (RM) questionnaire management system (RMS) GUI. There are four main frames in the GUI that could be seen in Figure [[fig:RM_GUI]].
...
This frame holds information that are essential for all tools in other frames, and will be used all over this guide.
    ...
    * *Main working directory* is the path of the main folder where
      ...
      the folder that contains all the software, from code to documentation. For more details see [[Software folder tree][Software folder tree]].
...
#+latex: \newpage
#+begin_note
    * Only files with =.xlsx= extensions are inserted.
...
The series drop-down list has three options:
    + Reported (REP) :: where the original questionnaire is
    ...
    + Observed (OBS) :: where the user is advised to modify and validate the
       data. 
...
An original questionnaire is always inserted to the /Reported/ series, to move it the /Observed/ or /Estimated/ series, choose first the country and then year, and then click on the buttons in the =Move between databases= frame.
    ...
    * =OBS to EST= would move the data from /Observed/ to /Estimated/ series.
...
alphanumeric code (AC) only. Each way has its own drop-down list.
    ...
    + AC :: A drop-down list with all ACs that are in the questionnaire. By selecting one and clicking on the adjacent =Export= button a new Excel workbook is created that holds the data.
...
When any data is exported, they are exported based on two viewing modes, an =Edit= and a =Read only= mode. 
   ...
   + =Read only= mode :: facilitates the viewing of the data by
       ...
       insert a =Read only= mode file, the validation step will fail,
...
extract them using one of the following methods
    + Direct extraction :: By selecting from the drop-down list or
       typing the indicator ID (IND_ID) and clicking on the adjacent =Extract=
...
#+begin_note
    ...
    * If indicators are incalculable because of missing data or
      inclusions, the figure will be replaced by resulting magnitude code in the respective cell. *If the cell is empty, either the
      indicator is a national level indicator or the /Estimated/ series is empty.*
    * To understand indicator magnitude see [[Column operation][Column operation]] section for the discussion on algebras.
    * Indicators are always sorted alphabetically.
    * Exported indicator files follow the naming convention =County Name-Year-Indic-indicator name.xlsx=, when the use of wildcards, =%= is replaced with =xx=.
#+end_note  
...
conditions are saved and also printed to the Status panel:
    + In case of an original questionnaire format :: checks are
    ...
    + In case of an =Edit= mode format :: checks are
...
The following is the schema of the SQL table, where 
    ...
    * =ADM_CODE= is the administrative division code.
...
CREATE TABLE METER_AUDIT_TRAIL(
	Audit_ID Integer , -- auto-increment ID
...
files:
 ...
 + *validation report*: =RMS\Log\Lao People's Democratic Republic_16-01-22-13-38_validation.txt=,
...
[[src:export_clean]], and the Excel workbook should look like Figure [[fig:export_clean]]. Notice
...
* Indicators
  ...
  * The superscript =L= means that the value from the previous year is being taken. For example, if the emco year is 2015 T.1^{L} means the 2014 value of T.1.
...
** Percentage of female teachers
   ...
   + IND_IDS information ::
   #+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID  | FORMULA             | H & L |
        |----------+---------+---------------------+-------|
        | ISCED 1  | FTP.1   | T.1.F/T.1           | yes   |
        | ISCED 2  | FTP.2   | T.2.GPV.F/T.2.GPV   | yes   |
        | ISCED 3  | FTP.3   | T.3.GPV.F/T.3.GPV   | yes   |
        | ISCED 23 | FTP.2t3 | T.23.GPV.F/T.23.GPV | yes   |
        |----------+---------+---------------------+-------|
	...
	#+ATTR_LATEX:  :align |l|l|l|c|
        ...
        | ISCED 2  | FTP.2.Pu   | T.2.GPV.Pu.F/T.2.GPV.Pu   | yes   |
        | ISCED 3  | FTP.3.Pu   | T.3.GPV.Pu.F/T.3.GPV.Pu   | yes   |
        | ISCED 23 | FTP.2t3.Pu | T.23.GPV.Pu.F/T.23.GPV.Pu | yes   |
        |----------+------------+---------------------------+-------|
	#+latex: \\
	#+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID     | FORMULA                   | H & L |
        ...
        | ISCED 2  | FTP.2.Pr   | T.2.GPV.Pr.F/T.2.GPV.Pr   | yes   |
        | ISCED 3  | FTP.3.Pr   | T.3.GPV.Pr.F/T.3.GPV.Pr   | yes   |
        | ISCED 23 | FTP.2t3.Pr | T.23.GPV.Pr.F/T.23.GPV.Pr | yes   |
        |----------+------------+---------------------------+-------|
...
** Percentage of newly recruited teachers
   ...
   #+ATTR_LATEX:  :align |l|l|l|c|
        ...
        | ISCED 2  | NTP.2   | NT.2.GPV/T.2.GPV   | yes   |
        | ISCED 3  | NTP.3   | NT.3.GPV/T.3.GPV   | yes   |
        | ISCED 23 | NTP.2t3 | NT.23.GPV/T.23.GPV | yes   |
        |----------+---------+--------------------+-------|
	...
	#+ATTR_LATEX:  :align |l|l|l|c|
        ...
        | ISCED 2  | NTP.2.Pu   | NT.2.GPV.Pu/T.2.GPV.Pu   | yes   |
        | ISCED 3  | NTP.3.Pu   | NT.3.GPV.Pu/T.3.GPV.Pu   | yes   |
        | ISCED 23 | NTP.2t3.Pu | NT.23.GPV.Pu/T.23.GPV.Pu | yes   |
        |----------+------------+--------------------------+-------|
	#+latex: \\
	#+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID     | FORMULA                  | H & L |
        ...
        | ISCED 2  | NTP.2.Pr   | NT.2.GPV.Pr/T.2.GPV.Pr   | yes   |
        | ISCED 3  | NTP.3.Pr   | NT.3.GPV.Pr/T.3.GPV.Pr   | yes   |
        | ISCED 23 | NTP.2t3.Pr | NT.23.GPV.Pr/T.23.GPV.Pr | yes   |
        |----------+------------+--------------------------+-------|
...
** Pupils teachers ratio
   ...
   #+ATTR_LATEX:  :align |l|l|l|c|
	...
	#+ATTR_LATEX:  :align |l|l|l|c|
        ...
        | ISCED 2  | PTRHC.2.Pu   | E.2.GPV.Pu/T.2.GPV.Pu   | yes   |
        | ISCED 3  | PTRHC.3.Pu   | E.3.GPV.Pu/T.3.GPV.Pu   | yes   |
        | ISCED 23 | PTRHC.2t3.Pu | E.23.GPV.Pu/T.23.GPV.Pu | yes   |
        |----------+--------------+-------------------------+-------|
	#+latex: \\
	#+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID       | FORMULA                 | H & L |
        ...
        | ISCED 2  | PTRHC.2.Pr   | E.2.GPV.Pr/T.2.GPV.Pr   | yes   |
        | ISCED 3  | PTRHC.3.Pr   | E.3.GPV.Pr/T.3.GPV.Pr   | yes   |
        | ISCED 23 | PTRHC.2t3.Pr | E.23.GPV.Pr/T.23.GPV.Pr | yes   |
        |----------+--------------+-------------------------+-------|
...
** Percentage of trained teachers
   ...
   #+ATTR_LATEX:  :align |l|l|l|c|
        ...
        | ISCED 2  | TRTP.2.Pu   | T.2.GPV.Pu.trained/T.2.GPV.Pu   | yes   |
        | ISCED 3  | TRTP.3.Pu   | T.3.GPV.Pu.trained/T.3.GPV.Pu   | yes   |
        | ISCED 23 | TRTP.2t3.Pu | T.23.GPV.Pu.trained/T.23.GPV.Pu | yes   |
        |----------+-------------+---------------------------------+-------|
	#+latex: \\
	#+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID      | FORMULA                         | H & L |
        ...
        | ISCED 2  | TRTP.2.Pr   | T.2.GPV.Pr.trained/T.2.GPV.Pr   | yes   |
        | ISCED 3  | TRTP.3.Pr   | T.3.GPV.Pr.trained/T.3.GPV.Pr   | yes   |
        | ISCED 23 | TRTP.2t3.Pr | T.23.GPV.Pr.trained/T.23.GPV.Pr | yes   |
        |----------+-------------+---------------------------------+-------|
	#+latex: \\
	#+ATTR_LATEX:  :align |l|l|l|c|
   |          | IND_ID       | FORMULA                           | H & L |
   ...
   | ISCED 2  | TrNTP.2.Pu   | NT.2.GPV.Pu.trained/NT.2.GPV.Pu   | yes   |
   | ISCED 3  | TrNTP.3.Pu   | NT.3.GPV.Pu.trained/NT.3.GPV.Pu   | yes   |
   | ISCED 23 | TrNTP.2t3.Pu | NT.23.GPV.Pu.trained/NT.23.GPV.Pu | yes   |
   |----------+--------------+-----------------------------------+-------|
   #+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c|
   |          | IND_ID       | FORMULA                           | H & L |
   ...
   | ISCED 2  | TrNTP.2.Pr   | NT.2.GPV.Pr.trained/NT.2.GPV.Pr   | yes   |
   | ISCED 3  | TrNTP.3.Pr   | NT.3.GPV.Pr.trained/NT.3.GPV.Pr   | yes   |
   | ISCED 23 | TrNTP.2t3.Pr | NT.23.GPV.Pr.trained/NT.23.GPV.Pr | yes   |
   |----------+--------------+-----------------------------------+-------|
...
** Percentage of permanent teachers
   This has three sets of symbols. One for the private sector, one
   for the public sector and one combined.
   + Python method ::  percentage_non_permanent_teachers
   + IND_IDS information Private ::
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c| 
        ...
        | ISCED 23 | PermTP.2t3.GPV.Pr | T.23.GPV.Pr.Perm/T.23.GPV.Pr | no    |
        |----------+------------------+-----------------------------+-------|
...
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c| 
        ...
        | ISCED 23 | PermTP.2t3.GPV.Pu | T.23.GPV.Pu.Perm/T.23.GPV.Pu | no    |
        |----------+------------------+-----------------------------+-------|
...
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c| 
        ...
        | ISCED 23 | PermTP.2t3.GPV | (T.23.GPV.Pu.Perm+T.23.GPV.Pr.Perm)/(T.23.GPV.Pr+T.23.GPV.Pu) | no    |
        |----------+---------------+-------------------------------------------------------------+-------|
...
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID        | FORMULA                                                           | H & L |
        |----------+---------------+-------------------------------------------------------------------+-------|
        | ISCED 1  | MAge.T.1      | \sum_{y in cat} T.1.y \times MP.y / \sum_{y in cat} T.1.y         | No    |
        | ISCED 2  | MAge.T.2.GPV  | \sum_{y in cat} T.2.GPV.y \times MP.y/ \sum_{y in cat} T.2.GPV.y  | No    |
        | ISCED 3  | MAge.T.3.GPV  | \sum_{y in cat} T.3.GPV.y \times MP.y/\sum_{y in cat} T.3.GPV.y   | No    |
        | ISCED 23 | MAge.T.23.GPV | \sum_{y in cat} T.23.GPV.y\times MP.y/ \sum_{y in cat} T.23.GPV.y | No    |
        |----------+---------------+-------------------------------------------------------------------+-------|
...
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c|
        |          | IND_ID        | FORMULA                                                            | H & L |
        |----------+---------------+--------------------------------------------------------------------+-------|
        | ISCED 1  | MExp.T.1      | \sum_{y in cat} T.1.y \times \times MP.y/ \sum_{y in cat} T.1.y    | No    |
        | ISCED 2  | MExp.T.2.GPV  | \sum_{y in cat} T.2.GPV.y\times MP.y / \sum_{y in cat} T.2.GPV.y   | No    |
        | ISCED 3  | MExp.T.3.GPV  | \sum_{y in cat} T.3.GPV.y\times MP.y / \sum_{y in cat} T.3.GPV.y   | No    |
        | ISCED 23 | MExp.T.23.GPV | \sum_{y in cat} T.23.GPV.y\times MP.y / \sum_{y in cat} T.23.GPV.y | No    |
        |----------+---------------+--------------------------------------------------------------------+-------|
...
#+latex: \\
   #+ATTR_LATEX:  :align |l|l|l|c|                       
     ...
     |--------------------------------+-----------+------------------------------------------------+-------|
        =IS= stands for ISCED level which are =[1, 2.GPV, 3.GPV, 23.GPV]=, combined with suffixes =['.Pu', '.Pr']= for public and private sub-category.
...
#+latex: \\
   ...
   Much notations are omitted for clearer presentation.
           ...
           * =E.23.GPV= is not collected in the questionnaire, so it is =E.2.GPV + E.3.GPV=.
           * To compute age 50 plus, data in ages 50 to 59 and 60 plus are added. This notation is omitted in the formula for brevity.
           * =Math=, =Reading and Writing= are only calculated for ISCED level 2t3, total.
           * =ISp1m= reads as ISCED level which is 1 level above or less than the actual level, that is for ISCED 2 =ISp1m = 3m=, and for ISCED 2t3 =ISp1m =4m=. The levels are summed.
...
def compute_all_indicators(self):
        """ Execute functions to calculate indicators."""
        ...
        ## Moving changed values to Audit trail
   ...
   #+BEGIN_SRC python :session example1 :export code :tangle ind_test1.py
     ...
     os.chdir(path_to_main_RMS_folder) # modify here.
...
#+END_SRC 
    ...
    + =RMS/Libraries/algebra-div.csv= for the division algebra.
...
** Example
   ...
   use the =column_operation= and the =div= auxiliary method, with the template provided in the end of [[Setup template]] section.
...
+ check_adm1_names :: Returns =True= if the name of each
     administrative division is filled and =False= otherwise.
+ check_reference_year :: Returns =True= if the reference year (Cell
     =M14= in the Policy Information sheet) is filled in the
...
+ check_values :: Checks that all the values reported in the
     ...
     A,a,N,n,Z,z,M,m. If this is the case it returns =True=, otherwise it
     returns =False=.
...
+ Improper inequalities :: 
   ...
   than others in different columns,the method *check_less* validates these cases. In this method there is a dictionary called =check_less_dictionary= whose
...
+ Incorrect region totals :: 
    The values of all the regions should add to the country total. The
...
value =No=, this only applies to imports in original questionnaire format where =Checking sheet= exist.
...
** Python help files
   ...
   =pydoc3= for any script. In the [[Software folder tree][Software folder tree]] section, the
